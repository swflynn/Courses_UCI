...
We can now consider the class of ODEs with variable coefficients.
Unfortunately most variable coefficient ODEs cannot be solved analytically, except for some special cases. 
...
\subsection*{Differentials as Algebra}
A quick side note, solving seperable differential equations as we did above implies the differential operators can be treated algebraically. 
Although this is convenient it is not 'kosher' we should probably prove it works.
Consider a simple differntial equation
\be
\begin{split}
    f(y) \frac{dy}{dx} &= g(x)\\
...
\ee
Substitute in the chain rule into our equation we find
\be
\begin{split}
...
\end{split}
\ee
So for this specific case we can prove the general result (which happens to be consistent with assuming the differentials can be treated as operators). 
