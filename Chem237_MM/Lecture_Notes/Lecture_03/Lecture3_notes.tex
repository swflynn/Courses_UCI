...
\maketitle
\section*{Stuff}
General form of clairaut equation
\be
y - x y' = f(y')
\ee
\be
\frac{d}{dx} y' - x y' = \frac{d}{dx}f(y') \Rightarrow y' - y' - xy'' = \frac{df}{dy} \frac{dy'}{x} = f'(y') y''
\Rightarrow -xy'' = f'(y') y''
\ee
\be
y''(f'(y') + x) = 0
...
\ee
\be
\int_{0}^{x} y'' dx = \int_{0}^{x} 0 dx \Rightarrow y' = b \\
\int_{0}^{x} y' dx = \int_{0}^{x} b dx \Rightarrow y = ax + b
\ee
\be
y - x(a) = f(a) \Rightarrow y = ax + f(a)
\ee
Example problem
\be
y = x y' + (y')^2
\ee
2nd order inhomogenous non constant coeeficient linear ODE
\begin{enumerate}
  \centering
