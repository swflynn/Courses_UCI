...
The homework will be split into both analytical and numerical problems, any programming language is fine for the numerical problems.
However, mathematica and other highly developed languages are discouraged.
...
f_3 = c_1f_1 + c_2f_2 .
\ee
This implies that f$_1$ and f$_2$ are linearly independent functions, meaning that there cannot exist a constant c such that $f_1 = cf_2$.
Checking if f$_1$ and f$_2$ are two linearly independent solutions can be done by determining the wronskian of the two functions, but we will not worry about having to do this.
...
%==============================================================================%
% Someone go find this theorem. C
%==============================================================================%
...
%==============================================================================%
% Not sure if the equation below could be more "correct" or if it's fine the way it is.
%==============================================================================%
\be
\sum_{n=0}^N a_n x^{(n)} = 0 \xrightarrow{\text{degenerate roots}} \sum_{n=0}^N a_n \lambda^n = 0 .
\ee
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.7]{under_damped.png}
  \caption{The real part of the solution, in blue, for the case of the un-driven, under-damped harmonic oscillator.
  The green dotted line shows that the amplitude of the function (and the the function itself) slowly decays to zero over time.}
...
%==============================================================================%
% Particular solution should be defined as x_p so it's easier to refer to.
%==============================================================================%
\be
...
The following cases show you how to approach obtaining the particular solution using this method (assuming that you have already solved for the homogenous solution).
...
Now, let's consider the example ODE
\be
...
\ee
I will leave it to you to check that this is a solution to the ODE.
...
\end{split}
\ee
Let's again consider the example ODE
\be
3 \frac{d^2 x}{dt^2} + x = (2-t)(t+2) .
\ee
Since $(2-t)(t+2) = 4 - t^2$, we can assume our guess is of the form $x_p(t)$ = $A_2t^2 + A_1t + A_0$.
Just like in case 3, we want to make sure our guess is as general as reasonably possible in order to avoid having to make another guess.
We continue with the method with substitution into the original ODE
\be
...
And this is the last special case that Vladimir is aware of!
So it's easy to see that the Method of Undetermined Coefficients is a simple method to help solve inhomogenous equations with constant coefficients because it only requires you to make your guess for the particular solution mimic the form of f(t) and it's just algebra from there!

\subsection*{Linear ODE with Variable Coefficients}
%==============================================================================%
Whether this section needs a little more elaborating is up to you.
Vlad didn't really cover a lot. He just showed a bunch of stuff.
%==============================================================================%
...
In general we don't have a general approach to solve these problems and it is often done numerically rather than analytically. We can try to use methods mentioned before such as the Variation of Parameters or Method of Undetermined Coefficients but it is not guaranteed to always work.
There are even other methods that are usually covered in undergraduate differential equations courses, but we will not go over them in class.
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.7]{morse.png}
  \caption{ }
  \label{fig:morse}
\end{figure}
%==============================================================================%
% Someone expand the conversation around this potential in the caption.
%==============================================================================%
...
Some other examples of linear ODEs with variable coefficients are the Morse Potential (Figure \ref{fig:morse}), and a piecewise constant potential (Figure \ref{fig:piecewise}).
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.7]{piecewise.png}
  \caption{ }
  \label{fig:piecewise}
\end{figure}
%==============================================================================%
...
As the name implies, this system consists of having a pendulum attached to an object that is moving vertically and/or horizontally. Obtaining the equation of motion for the system is non-trivial and is often done using Lagrangian mechanics instead of Newtonian mechanics.
...
\ee
So you can see that since one of the coefficients in the equation depends on t, this makes the equation a second order, linear, homogenous, non-constant coefficient ODE.
...
%==============================================================================%
% Don't know what "focal point where the pendulum connects to the base to move you can generate a stable solution to the problem" means. Elaborate on it if anyone can.
%==============================================================================%
\end{document}
